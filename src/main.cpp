...
//     A = host_kernel_launcher.random_mat_binary_f32_1d(data_domain::trinary, M, K_bits, 123).value();
//     B = host_kernel_launcher.random_mat_binary_f32_1d(data_domain::trinary, K_bits, N, 321).value();
...
int main(){
    ...
    sandbox<sandbox_algorithm::binmatmul, device_driver::vulkan_native> executor;
    auto res = executor.run(data_domain::trinary, 16, 16, 64);
    ...
    auto results = res.value();
    std::cout 
        << "max_abs_err=" << results.max_abs_err << std::endl
        << "mismatches=" << results.mismatches << std::endl
        << "total_size=" << results.total_size << std::endl
    << std::endl;
