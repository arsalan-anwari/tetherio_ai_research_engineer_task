...
auto multiply_sequenced_vulkan_native(
    ...
    kernel_config kernel_opts = config.kernels["multiply"];

    struct KernelParams { 
        T value; u32 count; 
    } kernel_params { mull_factor, static_cast<u32>(d_buff.size_bytes / sizeof(T)) };
...
auto multiply_standalone_vulkan_native(
    ...
    struct KernelParams { 
        T value; u32 count; 
    } kernel_params { mull_factor, static_cast<u32>(out.size()) };
